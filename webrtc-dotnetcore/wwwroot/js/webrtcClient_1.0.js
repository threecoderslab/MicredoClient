...
connection.start().then(function () {
    ...
    connection.on('joined', function (roomId) {
        ...
        myRoomId = roomId;
        isInitiator = false;
        document.getElementById("joinRoomBtn").style.visibility = 'hidden';
    ...
    connection.invoke("GetRoomInfo", parseInt(orgId)).catch(function (err) {
        return console.error(err.toString());
    });
...
$("#joinRoomBtn").click(function () {
    ...
    if (hasRoomJoined) {
        ...
        $.ajax({
            method: 'POST',
            url: '/Micred/StartCall',
            data: { 'agentId': agentId, 'sessionId': sessionId }
        })
...
function onDataChannelCreated(channel) {
    ...
    channel.onopen = function () {
        ...
        connectionStatusMessage.innerText = 'Կապը հաստատված է:';
...
sendButton.addEventListener("click", function () {
    ...
    var $inputs = $('#dataChannelSend :input');
    var controls = [];
    $inputs.each(function () {
        controls.push({ "controlId": $(this).attr('id'), "Name": $(this).attr('name'), "Val": $(this).val() });
    });
    console.log(typeof JSON.stringify(controls));
    dataChannel.send(JSON.stringify(controls));
});